...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        assert image_url_col in df.columns or image_path_col in df.columns, (
            f'DataFrame must contain either "{image_url_col}" or "{image_path_col}" column.'
        )
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            + [
                {
                    "role": "user",
                    "content": [{"type": "image", "image": image}, {"type": "text", "text": self.prompts[idx]}],
                }
            ],
        )
...
class Evaluator:
        ...
        with open(result_file, "w") as f:
            ...
            for indices, prompts, failed in dataloader:
                ...
                if prompts:
                    ...
                    for idx, count in zip(indices, self.eval_batch(prompts), strict=False):
    ...
    def eval_batch(self, prompts: list) -> list:
        ...
        return [self.eval_single(prompt) for prompt in prompts]
