...
def load_and_merge(dfile: Path, rfile: Path) -> pd.DataFrame:
    ...
    """
    df_truth = pd.read_csv(dfile).reset_index(drop=False).rename(columns={"index": "idx"})
    ...
    for data_path in data_dir.glob("*/dataset*.csv"):
        try:
            ...
            try:
                ...
                df = df.astype(
                    {"idx": int, "file_name": str, "prompt": str, "truth": int, "label": str}, errors="raise"
            ...
            for model_dir in sorted(results_dir.iterdir()):
                ...
                try:
                    ...
                    rd_df = rd_df.astype({"idx": int, "result": int, "model": str})

                    # Check if model indices match dataset indices
                    missing_indices = set(df["idx"]) - set(rd_df["idx"])
                    if missing_indices:
                        print(
                            f"Warning: Model {model_dir.name} is missing {len(missing_indices)} indices for {dataset_name}"
                        )
            ...
            if model_results:
                ...
                try:
                    final_df = df.merge(
                        all_results.rename(columns={"result": "model_result"}),
                        on="idx",
                        how="left",
...
def find_largest_outliers(df, n=10, model=None, dataset=None, outfile: Path = ANALYSIS_DIR / "outliers.csv"):
    """Save top-n absolute-error outliers to a CSV file."""
    df_out = df.assign(abs_err=(df["result"] - df["truth"]).abs())
    df_top = (
        df_out.nlargest(n, "abs_err")
        .reset_index()
        .rename(columns={"index": "idx"})[["idx", "truth", "result", "abs_err"]]
    )
    df_top["model"] = model
...
def find_label_performance(df, n=10, model=None, dataset=None, outfile: Path = ANALYSIS_DIR / "label_accuracy.csv"):
    """Save top/worst-n label accuracies to a CSV file."""
    acc = df.groupby("label").apply(lambda g: (g["result"] == g["truth"]).mean()).reset_index(name="accuracy")
    ...
    for data_path in data_dir.glob("*_dataset.csv"):
        ...
        for i, model_dir in enumerate(models):
            ...
            ax2.plot(
                ...
                linestyle="-",
                linewidth=1,
                label=f"{model_dir.name} mae",
            )

        ax1.set(
            ylim=(0, 1), xticks=x + width * (len(models) - 1) / 2, xticklabels=labels, ylabel="Exact-match accuracy"
        )
...
def plot_best_stacked(data_dir=DATA_DIR, results_dir=RESULTS_DIR, n_bins=25, max_val=120):
    ...
    for data_path in data_dir.glob("*_dataset.csv"):
        ...
        width = 0.6
        # Create stacked bars
        bars_acc = ax.bar(x, best_acc, width=width, label="Accuracy (higher is better)", color="green", alpha=0.8)
        bars_under = ax.bar(
            x,
            best_under,
            width=width,
            bottom=best_acc,
            label="Underprediction (lower is better)",
            color="orange",
            alpha=0.8,
        )
        bars_over = ax.bar(
            ...
            best_over,
            width=width,
            bottom=best_acc + best_under,
            label="Overprediction (lower is better)",
            color="red",
            alpha=0.8,
        )
        # Add model name labels to accuracy segment
        for i, (bar, model) in enumerate(zip(bars_acc, best_model, strict=False)):
            ...
            if height > 0.1:  # If accuracy segment is large enough
                ...
                ax.text(i, 0.02, model, ha="center", va="bottom", rotation=90, fontsize=8, color="black")
        ax.set(ylim=(0, 1), xticks=x, xticklabels=labels, ylabel="Rate")
...
def plot_best_models_per_bin(data_dir=DATA_DIR, results_dir=RESULTS_DIR, n_bins=25, max_val=120):
    ...
    bars_acc = ax.bar(x, acc_values, width=width, label="Accuracy", color="forestgreen", alpha=0.9)
    bars_under = ax.bar(
        x, under_values, width=width, bottom=acc_values, label="Underprediction", color="royalblue", alpha=0.9
    )
    bars_over = ax.bar(
        x,
        over_values,
        width=width,
        bottom=acc_values + under_values,
        label="Overprediction",
        color="crimson",
        alpha=0.9,
    )
    # Add best model names to the bars
    for i, (acc, model) in enumerate(zip(acc_values, best_model_names, strict=False)):
        if acc > 0.15:  # Only add text if there's a visible bar
            ax.text(
                i, acc / 2, model, ha="center", va="center", rotation=90, fontsize=8, color="white", fontweight="bold"
            )
        elif model:
            ax.text(i, 0.02, model, ha="center", va="bottom", rotation=90, fontsize=8, color="black")
    ...
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
    plt.title(f"Best Model Performance per Bin Across All Datasets (bw={bw})", fontsize=14, fontweight="bold")
    plt.legend(loc="upper right")
...
def plot_model_accuracies_by_bin(
    csv_path, bin_width, value_range, log_scale=False, minData=3, output_path="accuracy_plot.pdf"
):
    ...
    for i, model in enumerate(pivot_mean.columns):
        ...
        ax.plot(bin_centers, plot_data, label=model, marker="o", markersize=2, linewidth=1, color=cmap(i % cmap.N))
...
def plot_label_performance(csv_path, minData=5, maxTruth=20):
    ...
    label_overall_performance = (
        label_accuracy[label_accuracy["label"].isin(valid_labels)]
        .groupby("label")["exact_match_rate"]
        .mean()
...
def plot_accOverUnder(csv_path, bin_width, value_range, log_scale=False, minData=3, output_path="accuracy_plot.pdf"):
    ...
    grouped = (
        df.groupby(["model", "bin"], observed=False)
        .agg(
            {
    ...
    grouped.columns = ["_".join(col).rstrip("_") if isinstance(col, tuple) else col for col in grouped.columns.values]
    ...
    for i, model in enumerate(pivot_mean.columns):
        ...
        acc_line = ax.plot(bin_centers, plot_data, label=model, marker="o", markersize=2, linewidth=1, color=color)[0]

        # Plot underprediction (dotted line)
        ax.plot(bin_centers, plot_under, linestyle=":", linewidth=1, color=color)

        # Plot overprediction (dashed line)
        ax.plot(bin_centers, plot_over, linestyle="--", linewidth=1, color=color)
    ...
    avg_acc_line = ax.plot(
        bin_centers, avg_accuracy, label="Average Accuracy", linestyle="-", linewidth=2, color="black"
    )[0]

    # Plot average underprediction (dotted black line)
    avg_under_line = ax.plot(bin_centers, avg_under, linestyle=":", linewidth=2, color="black")[0]

    # Plot average overprediction (dashed black line)
    avg_over_line = ax.plot(bin_centers, avg_over, linestyle="--", linewidth=2, color="black")[0]
    ...
    ax.set_xticklabels(
        [
            f"[{interval.left:.0f},{(interval.right - 1):.0f}]"
...
def plot_prediction_trends_by_tolerance(
    csv_path,
    bin_width,
    value_range,
    tolerances=[0, 1, 2],
    log_scale=False,
    minData=5,
    output_path="prediction_trends.pdf",
):
    # Unpack the value_range tuple into min_value and max_value
    min_value, max_value = value_range
    ...
    for t_idx, tolerance in enumerate(tolerances):
        ...
        for pred_type, line_style in line_styles.items():
            ...
            bin_data = {
                bin_val: {"mean": mean, "count": count}
                for bin_val, mean, count in zip(grouped["bin"], grouped["mean"], grouped["count"], strict=False)
            }
    ...
    tolerance_legend = ax.legend(handles=dummy_tolerance_lines, loc="upper left", title="Tolerance Levels")
    ...
    ax.set_title(
        f"Prediction Trends by Ground Truth Value ({min_value}-{max_value}), Bin Width: {bin_width}", fontsize=14
    )
